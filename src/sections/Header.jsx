...
// adding a useEffect hook to track the scroll movement and trigger it when the scroll is more that 32 px 
    useEffect(()=>{
      const handleScroll = () => {
        setHasScrolled(window.scrollY > 32)
      };
// to optimize performance we remove the addEventListener 
  ...
  return (
    <header className={clsx('fixed top-0 left-0 z-50 w-full py-10 transition-all duration-500 max-lg:py-4',hasScrolled && 'py-2 bg-black-100 backdrop-blur-[8px]')}>
        <div className="container flex h-14 items-center max-lg:px-5">
            ...
            {/* cls is a package that allow us to construct classname string conditionally */}
            ...
            <nav className="max-lg:relative max-lg:z-2 max-lg:my-auto">
                <ul className="flex max-lg:block max-lg:px-12">
                    ...
                    <li className='nav-logo'>
                        <LinkScroll
                        to="hero" offset={-250} spy smooth className={clsx("max-lg:hidden transition-transform duration-500 cursor-pointer")}
                        >
